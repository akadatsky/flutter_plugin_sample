...
class InkResponse extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final parentState = _ParentInkResponseProvider.of(context);
...
class _InkResponseState extends State<_InkResponseStateWidget>
  ...
  void markChildInkResponsePressed(
    ...
    final lastAnyPressed = _anyChildInkResponsePressed;
    if (value!) {
      _activeChildren.add(childState);
    } else {
      _activeChildren.remove(childState);
    }
    final nowAnyPressed = _anyChildInkResponsePressed;
  ...
  void updateHighlight(_HighlightType type, {required bool? value}) {
    final highlight = _highlights[type];
  ...
  InteractiveInkFeature _createInkFeature(Offset globalPosition) {
    ...
    final color = widget.overlayColor?.resolve(pressed) ??
        widget.splashColor ??
        Theme.of(context).splashColor;
    final rectCallback =
        widget.containedInkWell ? widget.getRectCallback!(referenceBox) : null;
    final borderRadius = widget.borderRadius;
    final customBorder = widget.customBorder;
  ...
  bool? get _shouldShowFocus {
    final mode = MediaQuery.of(context).navigationMode;
    switch (mode) {
  ...
  void _startSplash({TapDownDetails? details, BuildContext? context}) {
    ...
    final splash = _createInkFeature(globalPosition);
  ...
  void deactivate() {
    ...
    for (final highlight in _highlights.keys) {
  ...
  Widget build(BuildContext context) {
    ...
    for (final type in _highlights.keys) {
